...
`ridl_datasets_list` is a list) of datasets using `R` function to access
...
`resources` using `list_resources`.
...
ct <- list_containers()
head(ct)
## [1] "afghanistan"      "africa"           "algeria"
## [4] "algeria-sens"     "algeria-wash-kap" "americas"

grep("niger-", ct, ignore.case = TRUE, value = TRUE)
## [1] "niger-protection" "niger-sens"

pull_container("niger-protection")
