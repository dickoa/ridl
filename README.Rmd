...
We can select a particular dataset from the list of datasets using `R` function to access elements from list (e.g `[[`). In this example, we will use `purrr::pluck` since it plays well with the pipe operator `%>%`. Once the dataset selected, it's possible to list all its resources using `list_dataset_resources`.
...
A `ridl_resources_list` is a simple `R` `list` and can be manipulated using `purrr::pluck` to select the one you want to read (`read_resource`) or download (`download_resource`).
