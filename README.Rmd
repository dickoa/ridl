...
We can select a particular dataset from the list of datasets using `R` function to access elements from list (e.g `[[`). In this example, we can use `purrr::pluck` or `dplyr::nth` since they play well with the pipe operator `%>%`. Once the dataset selected, it's possible to list all its resources using `list_resources`.
