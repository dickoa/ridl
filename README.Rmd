...
You can also configure directly the `ridl` package using the `set_ridl_config` function and check the config using `get_ridl_config` but it's not persistent if you close your session.
...
We can select a particular `dataset` from the list (a `ridl_dataset_list` is a list) of `dataset` using `R` function to access elements from list (e.g `[[`). In this example, we can use either `purrr::pluck` or `dplyr::nth` since they both play well with the pipe operator `%>%`. Once the dataset selected, it's possible to list all its `resource` objects using `list_ridl_resource`.
...
We can also use `pull_ridl_dataset` to directly read and access a dataset object.
